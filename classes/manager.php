...
/**
 * Class manager for openbook activity
 *
...
class manager {
    ...
    /**
     * Return the current count of users who have submitted files to this openbook module, that the current user can see.
     *
    ...
    public function count_all_users_answered(
        ...
        $select = $tableprefix . 'openbook = :openbookid';
        $params = [
            'openbookid' => $this->instance->id,
        ];
        if ($optionid) {
        ...
        if (empty($groupids)) {
            ...
            return $this->db->count_records_select('openbook_file', $select, $params, 'COUNT(DISTINCT userid)');
        }
        ...
        $query = "SELECT COUNT(DISTINCT of.userid)
                FROM {openbook_file} of, {groups_members} gm
               WHERE $select
    ...
    /**
     * Check if the current user has submitted in the openbook.
     *
     * @return bool true if the user has answered, false otherwise
     */
    public function has_answered(): bool {
        global $USER;
        $conditions = ['openbook' => $this->instance->id, 'userid' => $USER->id];
        return $this->db->record_exists('openbook_file', $conditions);
    }
}
